...
public class StatusBarUtil {
    ...
    private static final int FAKE_STATUS_BAR_VIEW_ID = Resources.getSystem().getIdentifier("statusbarutil_fake_status_bar_view", "id", "android");
    private static final int FAKE_TRANSLUCENT_VIEW_ID = Resources.getSystem().getIdentifier("statusbarutil_translucent_view", "id", "android");
     ...
     */
    public static int StatusBarLightMode(Activity activity, boolean dark, boolean fitsSystemWindows) {
        int result = 0;
    ...
    public static void setColor(Activity activity, @ColorInt int color, @IntRange(from = 0, to = 255) int statusBarAlpha, boolean fitsSystemWindows) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
...
//            activity.getWindow().addFlags(WindowManager.LayoutParams.FLAG_DRAWS_SYSTEM_BAR_BACKGROUNDS);
//            activity.getWindow().clearFlags(WindowManager.LayoutParams.FLAG_TRANSLUCENT_STATUS);
//            activity.getWindow().setStatusBarColor(calculateStatusColor(ContextCompat.getColor(mContext, R.color.transparent), 0));
//        } else
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
            activity.getWindow().addFlags(WindowManager.LayoutParams.FLAG_TRANSLUCENT_STATUS);
            ViewGroup decorView = (ViewGroup) activity.getWindow().getDecorView();
...
//    }
     ...
     * @param color    状态栏颜色值
     */
    public static void setColorNoTranslucent(Activity activity, @ColorInt int color, boolean fitsSystemWindows) {
     ...
     * @param activity 需要设置的 activity
     */
    public static void setTransparentColor(Activity activity) {
        setColor(activity);
    }
     ...
     * 设置根布局参数
     */
    private static void setRootView(Activity activity, boolean fitsSystemWindows) {
        ViewGroup parent = (ViewGroup) activity.findViewById(android.R.id.content);
        for (int i = 0, count = parent.getChildCount(); i < count; i++) {
            View childView = parent.getChildAt(i);
            if (childView instanceof ViewGroup) {
