 ...
 */
public class CameraDisplayDoubleInput extends BaseDisplay implements Renderer {
...
//                            LogUtils.i(TAG,"add tongue model result: %d", ret );
    ...
    private void initSticker() {
        ...
        //从资源文件加载Avatar模型
        long startLoadAvatarModel = System.currentTimeMillis();
        mStStickerNative.loadAvatarModelFromAssetFile(FileUtils.MODEL_NAME_AVATAR_CORE, mContext.getAssets());
        Log.e(TAG, "load model name: "+ FileUtils.MODEL_NAME_AVATAR_CORE + " cost time: " + (System.currentTimeMillis() - startLoadAvatarModel));
            ...
            if(mNeedBeautify || mNeedSticker || mNeedFaceAttribute) {
                ...
                if(mIsCreateHumanActionHandleSucceeded){
                    ...
                    if(mNeedDistance){
                        if(humanAction != null && humanAction.faceCount > 0){
                            ...
                            mFaceDistance = mSTHumanActionNative.getFaceDistance(humanAction.faces[0], getHumanActionOrientation(), mImageHeight, mImageWidth, mCameraProxy.getCamera().getParameters().getVerticalViewAngle());
                ...
                if (mNeedBeautify) {// do beautify
                    ...
                    if (!mNeedBeautyOutputBuffer) {
                        ...
                        result = mStBeautifyNative.processTextureAndOutputBuffer(textureId, mImageWidth, mImageHeight, orientation, humanAction, mBeautifyTextureId[0], outputImageBuffer, STCommon.ST_PIX_FMT_RGBA8888, mHumanActionBeautyOutput);
                ...
                if(mNeedSticker){
                    ...
                    */
                    ...
                    if((mParamType & STMobileStickerNative.ST_INPUT_PARAM_CAMERA_QUATERNION) == STMobileStickerNative.ST_INPUT_PARAM_CAMERA_QUATERNION && mSensorEvent != null && mSensorEvent.values != null && mSensorEvent.values.length > 0){
