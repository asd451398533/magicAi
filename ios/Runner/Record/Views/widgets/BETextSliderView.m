...
- (void)initDefaultSize {
    _activeLineColor = UIColor.blackColor;
    _inactiveLineColor = UIColor.blackColor;
...
//    [super touchesEnded:touches withEvent:event];
    // 取消延迟显示
    UITouch *touch = [touches anyObject];
    [self dispatchXEnd:[touch locationInView:self].x];
    _isInTouch = false;
...
- (void)dispatchXEnd:(CGFloat)x {
    ...
    if (_progress != progress) {
        ...
        if ([self.delegate respondsToSelector:@selector(progressDidChange:)]) {
            [self.delegate progressTouchEnd:_progress];
        }
        // 请求重绘
        if (!_isShowText) {
            [self setNeedsDisplay];
