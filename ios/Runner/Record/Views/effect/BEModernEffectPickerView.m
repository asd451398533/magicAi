...
/**
 备份当前的美颜状态
 */
@property (nonatomic, strong) BEEffectBackup *effectBackup;
@property (nonatomic, strong) BEModelProvider *modelProvider;
...
//            make.top.equalTo(self.mas_top).with.offset(-20);
    ...
    _effectBackup = [[BEEffectBackup alloc] init];
    _modelProvider = [[BEModelProvider alloc] init];
...
- (void)loadData {
    self.categories = [self effectCategoryModelArray];
...
- (void)be_showMakeupOptions:(BEEffectNode)type title:(NSString *)title isShow:(BOOL)show animation:(BOOL)animation {
    if (show) {
        ...
        self.vcMakeupOption = [self be_makeupOptionVC:type];
        [self.vcMakeupOption setType:type modelProvider:self.modelProvider];
...
- (BEFaceBeautyViewController *)be_makeupOptionVC:(BEEffectNode)type {
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if ([cell isKindOfClass:[BEEffectFaceBeautyViewCell class]]) {
        NSArray<NSNumber *> *array = [self effectNodeArray];
        BEEffectFaceBeautyViewCell *c = (BEEffectFaceBeautyViewCell *)cell;
        // 设置数据
        [c setType:[array[row] longValue] modelProvider:self.modelProvider];
        // 设置选中单位
...
- (void)switchTabDidSelectedAtIndex:(NSInteger)index {
    ...
    //美体，隐藏 slider
    self.textSlider.hidden = (!self.bodyEnable && index == 2);
    
    //每次切换tab的时候切换 currentSelectItem
    if (index < self.categories.count - 1) {
        NSNumber *curPage = [self effectNodeArray][index];
...
- (void)onClose {
    if (self.effectBackup.available) return;
    ...
    [self.effectBackup backup:&_currentSelectItem
           selectedNodeOfPage:&_selectedNodeofPage
 ...
 buttonItemModelWithIntensity:&_buttonItemModelWithIntensity
             savedIntensities:&savedIntensities
...
- (void)recoverEffect {
    if (!self.effectBackup.available) return;
    NSDictionary<NSNumber *, NSNumber *> *savedIntensities = nil;
    [self.effectBackup recover:&_currentSelectItem
            selectedNodeOfPage:&_selectedNodeofPage
               selectedNodeSet:&_selectedNodeSet
          buttonItemModelCache:&_buttonItemModelCache
  buttonItemModelWithIntensity:&_buttonItemModelWithIntensity
...
- (void)onBtnDefaultTap {
    if (self.onDefaultTapDelegate != nil) {
        [self.onDefaultTapDelegate onDefaultEffectTap];
    }
    self.effectBackup.available = NO;
    ...
    NSArray<BEButtonItemModel *> *array = [self.modelProvider buttonItemArrayWithDefaultIntensity];
...
- (BETextSliderView *)textSlider {
    if (!_textSlider) {
        ...
        _textSlider.textColor=UIColor.blackColor;
...
- (BEFaceBeautyViewController *)vcMakeupOption {
    ...
    return [BEFaceBeautyViewController new];
}
...
- (NSDictionary<NSNumber *,NSNumber *> *)defaultValue {
    return [self.modelProvider defaultValue];
}
...
- (NSArray<NSNumber *> *)effectNodeArray {
    ...
    if (!self.bodyEnable) {
        ...
        [copy removeObjectAtIndex:2];
        array = copy;
    }
    return array;
}

- (NSArray *)effectCategoryModelArray {
    NSArray *array = [BEEffectDataManager effectCategoryModelArray];
    if (!self.bodyEnable) {
        NSMutableArray *copy = [array mutableCopy];
        [copy removeObjectAtIndex:2];
        array = copy;
