...
- (void)fetchDataWithCompletion:(BEEffectDataFetchCompletion)completion {
    switch (self.type) {
        ...
        case BETypeAnimoji:
            ...
            break;
        case BETypeArscan:
            [self _fetchArscanDataWithCompletion:completion];
        default:
            break;
    }
}
...
- (void)_fetchArscanDataWithCompletion:(BEEffectDataFetchCompletion)completion {
    static NSArray *_arscanArray = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        _arscanArray = @[
            NSLocalizedString(@"arscan_merry", nil), @"icon_yanjlimdaidongxi", @"merry_chrismas", @"",
        ];
    });
    ...
    [self runAsync:^{
        ...
        responseModel.stickerGroup = [BEEffectDataManager be_genStickerArray:_arscanArray];
...
- (void) _fetchAnimojiDataWithCompletion:(BEEffectDataFetchCompletion)completion {
    ...
    dispatch_once(&onceToken, ^{
        _animojiArray = @[
        NSLocalizedString(@"animoji_boy", nil), @"icon_change_face", @"../animoji/animoji_boy", @"",
        NSLocalizedString(@"animoji_girl", nil), @"icon_change_face", @"../animoji/animoji_girl", @""
        ];
    });
    [self runAsync:^{
        ...
        NSError *error = nil;
        BEEffectResponseModel *responseModel = [BEEffectResponseModel new];
        responseModel.stickerGroup = [BEEffectDataManager be_genStickerArray:_animojiArray];
...
- (void) _fetchStickerDataWithCompletion:(BEEffectDataFetchCompletion)completion{
    static NSArray *_stickerArray = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        _stickerArray = @[
                   // @"控雨", @"icon_rain", @"rain",
    ...
    [self runAsync:^{
        ...
        NSError *error = nil;
        BEEffectResponseModel *responseModel = [BEEffectResponseModel new];
        responseModel.stickerGroup = [BEEffectDataManager be_genStickerArray:_stickerArray];
...
+ (NSArray *)be_genStickerArray:(NSArray *)array {
    ...
    for (int i = 0; i < array.count; i += 4) {
        ...
        sticker.title = array[i];
        sticker.filePath = array[i + 2];
        sticker.imageName = array[i + 1];
        sticker.toastString = array[i + 3];
